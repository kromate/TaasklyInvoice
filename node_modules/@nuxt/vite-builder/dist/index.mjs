...
import { resolve, join, dirname, normalize, isAbsolute, relative } from 'pathe';
import { logger, resolveModule, isIgnored, requireModule } from '@nuxt/kit';
import replace from '@rollup/plugin-replace';
import { sanitizeFilePath } from 'mlly';
import { getPort } from 'get-port-please';
import vuePlugin from '@vitejs/plugin-vue';
import viteJsxPlugin from '@vitejs/plugin-vue-jsx';
import { joinURL, withoutTrailingSlash, parseURL } from 'ufo';
...
async function prepareDevServerEntry(ctx) {
  ...
  await fse.writeFile(resolve(ctx.nuxt.options.buildDir, "dist/server/server.mjs"), `export { default } from ${JSON.stringify(pathToFileURL(serverResolvedPath).href)}`);
