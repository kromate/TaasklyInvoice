import { dirname, resolve, basename, extname, normalize, relative, isAbsolute, join } from 'pathe';
...
import { encodePath, parseURL, parseQuery, withQuery, joinURL } from 'ufo';
...
const pagesModule = defineNuxtModule({
  ...
  setup(_options, nuxt) {
    ...
    nuxt.hook("autoImports:extend", (autoImports) => {
      autoImports.push({ name: "definePageMeta", as: "definePageMeta", from: resolve(runtimeDir, "composables") }, { name: "useLink", as: "useLink", from: "vue-router" });
    });
    const macroOptions = {
...
async function initNuxt(nuxt) {
  ...
  nuxt.options.build.transpile.push(...nuxt.options._layers.filter((i) => i.cwd && i.cwd.includes("node_modules")).map((i) => i.cwd));
